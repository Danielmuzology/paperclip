...
export function normalizeIssueExecutionPolicy(input: unknown): IssueExecutionPolicy | null {
  ...
  const linkedWorkCompletion = parsed.data.linkedWorkCompletion;

  if (stages.length === 0 && !monitor && !reviewPreset && !authorizationPolicy && !linkedWorkCompletion) return null;
  ...
  return {
    ...
    ...(linkedWorkCompletion ? { linkedWorkCompletion } : {}),
  };
}
